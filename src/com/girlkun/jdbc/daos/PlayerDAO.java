...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            dataArray.add(0);
            String itemTimeNew = dataArray.toJSONString();
            dataArray.clear();
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, data_card, bill_data, info_phoban, data_may_do, info_achievement,data_duoi_khi, itemItem) "
                    ...
                    itemTime, itemTimeSC, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall,
                    dataSideTask, data_card, bill_data, info_phoban, itemMayDo, info_achive, dkhi, itemTimeNew);
    ...
    public static void updatePlayer(Player player) {
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                dataArray.clear();
                // data item may dò
                dataArray.add((player.itemTime.isUseMaydoCua
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeMaydoCua))
                        : 0));
                dataArray.add((player.itemTime.isUseMaydoTom
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeMaydoTom))
                        : 0));
                dataArray.add((player.itemTime.isUseMaydoOc
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeMaydoOc))
                        : 0));
                dataArray.add((player.itemTime.isUseMaydoCa
                        ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeMaydoCa))
                        : 0));
                String itemMayDo = dataArray.toJSONString();
                dataArray.clear();

                dataArray.add((player.itemTime.isUseSMTiemAn
                        ? (ItemTime.TIME_SUC_MANH - (System.currentTimeMillis() - player.itemTime.lastTimeUseSMTiemAn))
                        : 0));
                String time_Tugino = dataArray.toJSONString();
                dataArray.clear();
                ...
                String query = " update player set info_achievement =?, data_may_do = ?, khi_gas =?, data_item_time_sieu_cap = ?,head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, pointPvp=?, NguHanhSonPoint=?,data_card=?,bill_data =? ,info_phoban =? ,data_duoi_khi =?, itemItem =? where id = ?";
