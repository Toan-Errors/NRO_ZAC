...
//    public static byte RATE_EXP_SERVER = 1;// sau khi chinh
    ...
    public static final String queryTopSM = "SELECT id, CAST(SUBSTRING_INDEX(data_point, ',', -14) AS UNSIGNED) AS sm FROM player ORDER BY sm DESC LIMIT 100;";
 ...
 // load nhiem vu bo mong
            ...
            while (rs.next()) {
                ...
                //                itemTemp.name = "[" + itemTemp.id + "] " + rs.getString("name");
...
//                RUBY_REWARDS.add(Util.sendDo(861, 0, new ArrayList<>()));
            ...
            Logger.success("Load top KhiGas thành công (" + topSK.size() + ")\n");
