...
public class Controller implements IMessageHandler {
        ...
        try {
            ...
            switch (cmd) {
                ...
                case -105:
                    ...
                    } else if (player.type == 2 && player.maxTime == 10) {
                        ChangeMapService.gI().changeMapBySpaceShip(player, 135, -1, 35);
                        player.iDMark.setGoToBDKB(false);
                    } else if (player.type == 3 && player.maxTime == 10) {
                        ChangeMapService.gI().changeMapBySpaceShip(player, 146, -1, 35);
                        player.iDMark.setGoToKG(false);
                    }
                    break;
                case 42:
                ...
                case -63:
                    if (player != null) {
                        FlagBagService.gI().sendIconEffectFlag(player, _msg.reader().readUnsignedByte());
                    }
                    break;
                case -32:
    ...
    public void sendInfo(MySession session) {
        ...
        if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
            NpcService.gI().createTutorial(player, -1,
                    "Chào mừng " + player.name + " đến với ngọc rồng online server Seobi\n"
    ...
    private void sendThongBaoServer(Player player) {
        Service.gI().sendThongBaoFromAdmin(player, "Đã mở chức năng úp Capsule kì bí.\nChúc AE chơi game vui vẻ...");
    }
