...
public class NPoint {
    ...
    private void setHpMax() {
        ...
        // SM TIỀM ẨN
        if (this.player.itemTime != null && this.player.itemTime.isUseSMTiemAn) {
    ...
    private void setMpMax() {
        ...
        if (this.player.isPet && ((Pet) this.player).typePet == 2
                && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA
                        ...
                        || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA3
                        || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA4)) {
            this.mpMax += ((long) this.mpMax * 10 / 100);// MP berus
        }
        // hợp thể
        if (this.player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
    ...
    private void setCrit() {
        ...
        // SM TIỀM ẨN
        if (this.player.itemTime != null && this.player.itemTime.isUseSMTiemAn) {
            this.crit += (int) (this.crit * 50 / 100);
        }
        // biến khỉ
        if (this.player.effectSkill.isMonkey) {
            this.crit = 110;
    ...
    public int getDameAttack(boolean isAttackMob) {
        ...
        switch (skillSelect.template.id) {
            ...
            case Skill.DICH_CHUYEN_TUC_THOI:
                dameAttack *= 1;
                dameAttack = Util.nextInt((int) (dameAttack - ((long) dameAttack * 1 / 100)),
                        (int) (dameAttack + ((long) dameAttack * 1 / 100)));
        ...
        // Service.gI().sendThongBaoBenDuoi(player.name + " đã đánh 1 " +
        // player.playerSkill.skillSelect.template.name + " với sát thương " +
        // Util.getMoneys(dameAttack));
        ...
        // player.playerSkill.skillSelect.template.name + " với sát thương " +
        // Util.getMoneys(dameAttack));
        // } else if (skillSelect.template.id == Skill.QUA_CAU_KENH_KHI &&
        // player.isPl()) {
        // Service.gI().sendThongBaoBenDuoi(player.name + " đã đánh 1 " +
        // player.playerSkill.skillSelect.template.name + " với sát thương " +
        // Util.getMoneys(dameAttack));
        ...
        dameAttack = Util.nextInt((int) (dameAttack - ((long) dameAttack * 2 / 100)),
                (int) (dameAttack + ((long) dameAttack * 2 / 100)));
        if (player.isPl()) {
        ...
        if (dameAttack > 100_000_000) {
            if (skillSelect.template.id == Skill.KAMEJOKO && player.isPl()) {
                Service.gI().sendThongBaoBenDuoi(
                        player.name + " đã đánh một " + player.playerSkill.skillSelect.template.name
                                + " với sát thương " + Util.numberToMoney(dameAttack));
            } else if (skillSelect.template.id == Skill.LIEN_HOAN && player.isPl()) {
                Service.gI().sendThongBaoBenDuoi(
                        player.name + " đã đánh một " + player.playerSkill.skillSelect.template.name
                                + " với sát thương " + Util.numberToMoney(dameAttack));
            ...
            Logger.log(Logger.PURPLE, "Player: " + player.name + " Id: " + player.id + " Skill: "
                    + player.playerSkill.skillSelect.template.name + "Dame: " + Util.numberToMoney(dameAttack) + " \n");
        }
        return (int) dameAttack;
    }
