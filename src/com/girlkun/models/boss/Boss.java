...
public class Boss extends Player implements IBossNew, IBossOutfit {
    ...
    public void update() {
        ...
        switch (this.bossStatus) {
            ...
            case JOIN_MAP:
                ...
                // System.out.println("Map: " + this.zone.map.mapId + ", Zone: " +
                // this.zone.zoneId + ", Boss: " + this.name);
                break;
            case CHAT_S:
                if (chatS()) {
                    this.doneChatS();
    ...
    public void rest() {
        ...
        if (this.data[nextLevel].getTypeAppear() == TypeAppear.DEFAULT_APPEAR
                && Util.canDoWithTime(lastTimeRest, secondsRest * 300)) {
    ...
    protected void notifyJoinMap() {
        ...
        ServerNotify.gI().notify("BOSS " + this.name + " vừa xuất hiện tại " + this.zone.map.mapName + this.zone.zoneId
                + " (" + this.zone.map.mapId + ")" + "");
    }
    ...
    public void wakeupAnotherBossWhenAppear() {
        Logger.log(Logger.CYAN,
                "Boss " + this.name + " : " + this.zone.map.mapName + "(" + this.zone.map.mapId + ") khu: "
                        + this.zone.zoneId
                        + "\n");
        if (!MapService.gI().isMapMaBu(this.zone.map.mapId) && MapService.gI().isMapBlackBallWar(this.zone.map.mapId)) {
            System.out.println("BOSS " + this.name + " : " + this.zone.map.mapName + " khu vực " + this.zone.zoneId
                    + "(" + this.zone.map.mapId + ")");
        }
        if (this.bossAppearTogether == null || this.bossAppearTogether[this.currentLevel] == null) {
            return;
