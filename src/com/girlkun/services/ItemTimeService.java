...
public class ItemTimeService {
    ...
    public void sendAllItemTime(Player player) {
        ...
        if (player.itemTime.isUseSMTiemAn) {
            sendItemTime(player, 12086,
                    (int) ((TIME_SUC_MANH - (System.currentTimeMillis() - player.itemTime.lastTimeUseSMTiemAn))
                            / 1000));
        }
        ...
        if (player.itemTime.isdkhi) {
            sendItemTime(player, player.itemTime.icondkhi,
                    (int) ((TIME_DUOI_KHI - (System.currentTimeMillis() - player.itemTime.lastTimedkhi)) / 1000));
        }
        if (player.itemTime.isUseItemDeTu) {
            sendItemTime(player, 10850,
                    (int) ((TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeDeTu)) / 1000));
        }
    }
    ...
    public void sendTextDoanhTrai(Player player) {
        if (player.clan != null) {
            if (player.clan.doanhTrai != null && player.clan != null && !player.clan.doanhTrai_haveGone
                    && player.clan.doanhTrai_lastTimeOpen != 0) {
                int secondPassed = (int) ((System.currentTimeMillis() - player.clan.doanhTrai.getLastTimeOpen())
                        / 1000);
    ...
    public void sendTextBanDoKhoBau(Player player) {
        if (player.clan != null) {
            if (player.clan.banDoKhoBau != null && player.clan != null && !player.clan.banDoKhoBau_haveGone
                    && player.clan.banDoKhoBau_lastTimeOpen != 0) {
                int secondPassed = (int) ((System.currentTimeMillis() - player.clan.banDoKhoBau.getLastTimeOpen())
                        / 1000);
    ...
    // player.clan.lastTimeOpenKG) / 1000);
