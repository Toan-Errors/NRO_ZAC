...
public class TaskService {
    ...
    public void checkDoneTaskKillMob(Player player, Mob mob) {
        if (player == null && mob == null) {
            System.err.println("Error: Both player and mob are null");
            return;
        } else if (player == null) {
            System.err.println("Error: Player is null");
            // Xử lý khi player là null
            return;
        } else if (mob == null) {
            System.err.println("Error: Mob is null");
            // Xử lý khi mob là null
            return;
        } else {
            // Xử lý khi không có lỗi nào xảy ra
            if (!player.isBoss && !player.isPet && !player.isNewPet) {
                switch (mob.tempId) {
            ...
            if (player.playerTask.sideTask.isDone()) {
                ...
                switch (player.playerTask.sideTask.level) {
                    ...
                    case ConstTask.NORMAL:
                        rubyReward = Util.nextInt(10, 30);
                        break;
                    case ConstTask.HARD:
                    case ConstTask.VERY_HARD:
                        rubyReward = Util.nextInt(30, 70);
                        break;
                    case ConstTask.HELL:
                        rubyReward = Util.nextInt(70, 100);
                        break;
                }
                player.inventory.addRuby(rubyReward);
                Service.gI().sendMoney(player);
                Service.gI().sendThongBao(player, "Bạn nhận được "
                        + Util.numberToMoney(rubyReward) + " Ruby");
                player.playerTask.sideTask.reset();
            } else {
