...
public class Input {
        ...
        try {
            ...
            switch (player.iDMark.getTypeInput()) {
                ...
                case CHAN:
                    ...
                    int rubyCuoc = Integer.valueOf(text[0]);
                    if (rubyCuoc < 1000) {
                        Service.getInstance().sendThongBao(player, "Chỉ cược cao hơn 30 thỏi vàng.");
                        return;
                    }
                    if (rubyCuoc > 100000) {
                        Service.getInstance().sendThongBao(player, "Chỉ cược thấp hơn 1.000 thỏi vàng.");
                        return;
                    }
                    if (rubyCuoc <= 0) {
                        Service.getInstance().sendThongBao(player, "?");
                        ...
                        try {
                            if (thoiVang.quantity >= rubyCuoc) {
                                // player.inventory.ruby -= rubyCuoc;
                                InventoryServiceNew.gI().subQuantityItemsBag(player, thoiVang, rubyCuoc);
                                InventoryServiceNew.gI().sendItemBags(player);
                                Service.gI().sendMoney(player);
                                player.cuoc = rubyCuoc;
                ...
                case LE:
                    ...
                    rubyCuoc = Integer.valueOf(text[0]);
                    if (rubyCuoc < 1000) {
                        Service.getInstance().sendThongBao(player, "Chỉ cược cao hơn 30 thỏi vàng.");
                        return;
                    }
                    ...
                    if (goldTrade <= 0 || goldTrade >= 500) {
                        ...
                        Service.gI().sendThongBao(player,
                                "Số tiền của bạn là " + player.getSession().coinBar + " không đủ để quy "
                                        ...
                                        + (coindoi - player.getSession().coinBar));
                    }
                    break;
                    ...
                    if (RubyTrade <= 0 || RubyTrade >= 100000) {
                        ...
                        Service.gI().sendThongBao(player,
                                "Số tiền của bạn là " + player.getSession().coinBar + " không đủ để quy "
                                        ...
                                        + (RubyTrade - player.getSession().coinBar));
                    }
                    break;
                case SEND_ITEM:
                    if (player.isAdmin()) {
    ...
    public void createFormGiftCode(Player pl) {
        createForm(pl, GIFT_CODE, "Gift code Ngọc Rồng ZAC", new SubInput("Gift-code", ANY));
    }
    ...
    public void addItemGiftCodeToPlayer(Player p, final String giftcode) {
        ...
        try {
            red = GirlkunDB.executeQuery(
                    "SELECT * FROM `gift_code` WHERE `code` LIKE '" + Util.strSQL(giftcode) + "' LIMIT 1;");
            if (red != null && red.first()) {
                ...
                if (type == 1) {
                    for (int i = 0; i < listUser.size(); ++i) {
                        ...
                        if (playerId == p.id) {
                            NpcService.gI().createTutorial(p, 15269,
                ...
                listItem.clear();
                listUser.add(p.id);
                GirlkunDB.executeUpdate("UPDATE `gift_code` SET `limit` = " + limit + ", `listUser` = '"
