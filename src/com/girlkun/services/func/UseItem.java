...
public class UseItem {
                    ...
                    switch (item.template.id) {
                        ...
                        case 361:
                            ...
                            NpcService.gI().createMenuConMeo(pl, ConstNpc.CONFIRM_TELE_NAMEC, -1,
                                    "1 Sao (" + NgocRongNamecService.gI().getDis(pl, 0, (short) 353) + ")\n2 Sao ("
    ...
    private void openWoodChest(Player pl, Item item) {
        ...
        if (time != 0) {
             ...
             * RewardService.gI().initBaseOptionClothes(itemReward.template.id,
             * itemReward.template.type, itemReward.itemOptions);
             * RewardService.gI().initStarOption(itemReward, new
             * RewardService.RatioStar[]{new RewardService.RatioStar((byte) 1, 1, 2), new
             * RewardService.RatioStar((byte) 2, 1, 3), new RewardService.RatioStar((byte)
             * 3, 1, 4), new RewardService.RatioStar((byte) 4, 1, 5),});
    ...
    private void useItemTime(Player pl, Item item) {
        switch (item.template.id) {
            case 1992:
                pl.itemTime.lastTimeUseSMTiemAn = System.currentTimeMillis();
                pl.itemTime.isUseSMTiemAn = true;
                pl.itemTime.levelTiemAn = 1;
                Service.gI().player(pl);
