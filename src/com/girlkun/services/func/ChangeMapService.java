...
public class ChangeMapService {
        ...
        try {
            ...
            switch (pl.iDMark.getTypeChangeMap()) {
                ...
                case ConstMap.CHANGE_BLACK_BALL:
                    list = (pl.mapBlackBall = MapService.gI().getMapBlackBall());
                    msg.writer().writeByte(list.size());
                    for (int i = 0; i < pl.mapBlackBall.size(); i++) {
                        Zone zone = pl.mapBlackBall.get(i);
    ...
    private void changeMap(Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
        ...
        if (zoneJoin != null) {
            ...
            // System.err.println("zoneeeeeeeeeeee nullllllllllllllllllllllllll");
        }
    }
    ...
    public void changeMapWaypoint(Player player) {
        ...
        if (zoneJoin != null) {
            ...
            if (MapService.gI().isMapNha(zoneJoin.map.mapId)) {
                if (!player.isPet && !player.isBoss && player.idNRNM != -1) {
                    ItemMap itemMap = new ItemMap(player.zone, player.idNRNM, 1, player.location.x, player.location.y,
                            -1);
