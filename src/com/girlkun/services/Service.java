...
public class Service {
    ...
    public void sendPetFollow(Player player, short smallId) {
        ...
        try {
            ...
            if (smallId == 0) {
                ...
                if (smallId == 14420 || smallId == 14432 || smallId == 14434) {
    ...
    public void chat(Player player, String text) {
        ...
        if (player.getSession() != null && player.isAdmin()) {
            ...
            if (text.equals("ad")) {
                ...
                try {
                    ...
                    player.itemTime.isdkhi = true;
                    ItemTimeService.gI().removeItemTime(player, player.itemTime.icondkhi);
                    player.itemTime.icondkhi = 21839;
                ...
                try {
                    ...
                    sendThongBao(player, "" + player.name + " đã dịch chuyển tức thời đến: " + player.zone.map.mapName
                            + " (" + player.zone.map.mapId + ")");
                    return;
                } catch (Exception e) {
                    e.printStackTrace();
        ...
        // if (player.muav == false) {
        // player.muav = true;
        // Service.getInstance().sendThongBao(player, "Đã bật tự động mua vàng khi vàng
        // trên 500tr !");
        ...
        // player.muav = false;
        // Service.getInstance().sendThongBao(player, "Đã tắt tự động mua vàng khi vàng
        // trên 500tr !");
        // lasttimechatmuav = System.currentTimeMillis();
        // Logger.success("Thằng " + player.name + " chat muav\n");
        // return;
    ...
    public void chooseFlag(Player pl, int index) {
        if (MapService.gI().isMapBlackBallWar(pl.zone.map.mapId) || MapService.gI().isMapMaBu(pl.zone.map.mapId)
                || MapService.gI().isMapPVP(pl.zone.map.mapId) || MapService.gI().isnguhs(pl.zone.map.mapId)) {
    ...
    public void switchToRegisterScr(MySession session) {
