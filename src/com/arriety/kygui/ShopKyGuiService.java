...
public class ShopKyGuiService {
    ...
    public void buyItem(Player pl, int id) {
        ...
        if (it.goldSell > 0) {
            ...
            if (tv == null || tv.quantity < it.goldSell) {
                Service.getInstance().sendThongBao(pl, "Bạn không đủ thỏi vàng để mua vật phẩm này!");
                isBuy = false;
            } else {
                isBuy = true;
    ...
    public void upItemToTop(Player pl, int id) {
        ...
        NpcService.gI().createMenuConMeo(pl, ConstNpc.UP_TOP_ITEM, -1,
                "Bạn có muốn đưa vật phẩm ['" + ItemService.gI().createNewItem(it.itemId).template.name
                        + "'] của bản thân lên trang đầu?\nYêu cầu 20 thỏi vàng.",
                "Đồng ý", "Từ Chối");
    }

    public int upTopItem(int id) {
        for (ItemKyGui it : getItemKyGui()) {
            if (it != null && it.id == id) {
                it.isUpTop++;
    ...
    public List<ItemKyGui> getItemCanKiGui(Player pl) {
        List<ItemKyGui> its = new ArrayList<>();
        ShopKyGuiManager.gI().listItem.stream().filter((it) -> (it != null && it.player_sell == pl.id))
                .forEachOrdered((it) -> {
                    its.add(it);
                });
        pl.inventory.itemsBag.stream().filter((it) -> (it.isNotNullItem() &&
                ...
                // it.template.type == 11 || it.template.type == 5
                    ...
                    its.add(new ItemKyGui(InventoryServiceNew.gI().getIndexBag(pl, it), it.template.id, (int) pl.id,
    ...
    public byte getTabKiGui(Item it) {
        if (it.template.type >= 0 && it.template.type <= 2) {
            return 0;
        } else if ((it.template.type >= 3 && it.template.type <= 4) || it.template.type == 12
                || it.template.type == 33) {
            return 1;
    ...
    public void KiGui(Player pl, int id, int money, byte moneyType, int quantity) {
        try {
            ...
            if (tv == null || tv.quantity < 20) {
                Service.getInstance().sendThongBao(pl, "Bạn cần có ít nhất 20 thỏi vàng để làm phí");
                return;
            }
            ...
            if (money > 10000) {
                Service.getInstance().sendThongBao(pl, "Ký gửi tối đa 10.000 thỏi vàng");
                openShopKyGui(pl);
                return;
            }

            if (money < 10) {
                Service.getInstance().sendThongBao(pl, "Ký gửi tối thiểu 10 thỏi vàng");
                openShopKyGui(pl);
                return;
            }

            InventoryServiceNew.gI().subQuantityItemsBag(pl, tv, 20);
            InventoryServiceNew.gI().sendItemBags(pl);
            ...
            switch (moneyType) {
                case 0:// vàng
                    ...
                    ShopKyGuiManager.gI().listItem.add(new ItemKyGui(getMaxId() + 1, it.template.id, (int) pl.id,
                            getTabKiGui(it), money, -1, quantity, (byte) 0, it.itemOptions, false));
                    InventoryServiceNew.gI().sendItemBags(pl);
                    openShopKyGui(pl);
                    ShopKyGuiManager.gI().save();
                    Service.getInstance().sendMoney(pl);
                ...
                case 1:// hồng ngọc
                    InventoryServiceNew.gI().subQuantityItemsBag(pl, pl.inventory.itemsBag.get(id), quantity);
                    ShopKyGuiManager.gI().listItem.add(new ItemKyGui(getMaxId() + 1, it.template.id, (int) pl.id,
                            getTabKiGui(it), -1, money, quantity, (byte) 0, it.itemOptions, false));
                    InventoryServiceNew.gI().sendItemBags(pl);
                    openShopKyGui(pl);
                    ShopKyGuiManager.gI().save();
                    Service.getInstance().sendMoney(pl);
